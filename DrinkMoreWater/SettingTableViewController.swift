...
class SettingTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        dailyWaterTextField.delegate = self
        if let dailyWater = dailyWater {
            dailyWaterTextField.text = "\(dailyWater)"
        }
        
        if let notificationEnable = notificationEnable {
            notificationSwitch.isOn = notificationEnable
        }
...
extension SettingTableViewController: UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        return newLength <= 5 // replace 30 for your max length value
    }
}
